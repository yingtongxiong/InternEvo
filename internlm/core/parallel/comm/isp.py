...
class _SeqAllToAllOrigin(torch.autograd.Function):
    "sequence alltoall function"
    ...
    def backward(ctx, *grad_output: torch.Tensor) -> Tuple[None, torch.Tensor, None, None]:
        ...
        return (None, _SeqAllToAllOrigin.apply(ctx.group, *grad_output, ctx.gather_idx, ctx.scatter_idx), None, None)
...
class DistributedAttention(nn.Module):
    ...
    def _(self, q: torch.Tensor, kv: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # q = _SeqAllToAllOrigin.apply(self.spg, q, 2, 1)
        # kv = _SeqAllToAllOrigin.apply(self.spg, kv, 3, 1)
        torch.distributed.barrier()
        torch.cuda.synchronize()
        end_time = time.time()
        ...
        # context = _SeqAllToAllOrigin.apply(self.spg, context, 1, 2)
        torch.distributed.barrier()
        torch.cuda.synchronize()
        end_time = time.time()
